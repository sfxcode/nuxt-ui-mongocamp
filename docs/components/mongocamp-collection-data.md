...
- **Insert / Edit** — opens a form generated by [`useMongocampDynamicForm`](/composables/use-mongocamp-dynamic-form) from the schema inferred (via `useMongocampSchema`) from the currently-loaded page of rows — scalar fields, nested objects, and arrays (including arrays of objects, with add/clone/delete/drag row controls) all render as real inputs instead of raw JSON. Saving stamps `metaData` via `useMongocampDocument`'s `ensureMetaData`, then calls `documentApi.insert`/`documentApi.update` (the `_id` field is always stripped before sending, since MongoDB rejects updates to it). Editing preserves any document field the sampled schema never saw — `documentApi.update` is full-replace, so an unsampled field is carried over from the original row rather than dropped.
...
- [`useMongocampSchema`](/composables/use-mongocamp-schema) / [`useMongocampDynamicForm`](/composables/use-mongocamp-dynamic-form) — schema inference and generated-form power the Add/Edit modal
